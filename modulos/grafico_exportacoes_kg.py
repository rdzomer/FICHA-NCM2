...
def gerar_grafico_exportacoes(df, ncm_formatado, last_updated_month, last_updated_year):
    """
    Gera um gráfico de barras das exportações em KG para uma dada NCM, de 2010 a 2025,
    incluindo barras para 2024 (completo) e 2024 (parcial) correspondente a 2025 (parcial).

    Args:
        df (pd.DataFrame): DataFrame com os dados ('year', 'month', 'Exportações (KG)').
    ...
    try:
        ...
        df_plot['Exportações (KG)'] = pd.to_numeric(df_plot['Exportações (KG)'], errors='coerce').fillna(0)
        ...
        colors = {
            '2025 (Parcial)': 'midnightblue',
            '2024 (Parcial)': 'steelblue',
            '2024': 'skyblue',  # Cor para 2024 COMPLETO
            **{str(ano): 'skyblue' for ano in range(2010, 2024)}
        }

        # 7. Criação do Gráfico
        fig = px.bar(df_final, x='Tipo', y='Exportações (KG)',
                     color='Tipo',
                     color_discrete_map=colors,
                     title=f'Exportações (kg) da NCM {ncm_formatado}, de 2010 a 2025',
                     labels={'Tipo': 'Ano', 'Exportações (KG)': 'Exportações (KG)'})

        # 8. Formatação do Eixo Y (Corrigido - Pontos em vez de vírgulas)
        max_y = df_final['Exportações (KG)'].max()
        if max_y <= 50000000:
        ...
        print(f"Erro ao gerar gráfico de exportações: {e}")
        return None
